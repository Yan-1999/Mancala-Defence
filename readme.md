...
The difference between *fav.* and *vul. cells* is: Units on *vul. cells* will get **debuffed** (how?), but units on *fav. cells* stay unaffected.
...
*Units* can **not** be spawned on the *base*.
...
2. When the last *unit* is placed on a empty *cell*, player can instantly send all the *units* at the opposite *cell*. (How to determine "opposite"?)
...
There exists a *hand limit*. Once the limit is reached, the game will discard automatically (how?).
